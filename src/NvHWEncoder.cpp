...
void * handleAccept(void* arg){
	CNvHWEncoder* pthis = (CNvHWEncoder*)arg;
	socklen_t clilen;
	int connfd = 0;
	struct sockaddr_in cliaddr;
	for( ; ; ){
		clilen = sizeof(cliaddr);
		if( (connfd = accept(pthis->m_listenfd, (struct sockaddr*) &cliaddr, &clilen)) < 0){
			NvDebugLog("accept client connect error.");
			continue;
		}
		pthis->m_connfd = connfd;
		NvDebugLog("one client has connect serv");
	}
	return NULL;
}

int writen(int fd, const void *vptr, uint32_t n)
{
 	uint32_t nleft = 0, nwritten = 0;
 	const char *ptr;
 	ptr = (char*)vptr;
 	nleft = n;
 	while (nleft > 0) {
...
NVENCSTATUS CNvHWEncoder::NvEncUnmapInputResource(NV_ENC_INPUT_PTR mappedInputBuffer)
{
    ...
    if (mappedInputBuffer)
    {
        ...
        if (nvStatus != NV_ENC_SUCCESS)
        {
        	NvDebugLog("nvEncUnmapInputResource failed, nvStatus: %d", nvStatus);
			return nvStatus;
        }
    }
...
//    registerResParams.bufferFormat = NV_ENC_BUFFER_FORMAT_NV12_PL;
    ...
    if (nvStatus != NV_ENC_SUCCESS)
    {
    	printf("NvEncRegisterResource nvStatus: %d\n", nvStatus);
        assert(0);
    }
...
CNvHWEncoder::CNvHWEncoder()
{
	...
	m_listenfd = 0;
	m_connfd = 0;
	m_out_buffer = new uint8_t[2*1024*1024];
...
int CNvHWEncoder::Init_tcp_serv()
{
	struct sockaddr_in servaddr;
	int result = 0;
	pthread_t tid;
	if( (m_listenfd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0){
		NvDebugLog("listenfd create failed.");
		return m_listenfd;
	}
	...
	if( (result = setsockopt(m_listenfd, SOL_SOCKET, SO_REUSEADDR, &flag, len)) != 0){
		NvDebugLog("setsockopt reuseaddr error.");
		return result;
	}
	
	if( (result = bind(m_listenfd, (struct sockaddr*)&servaddr, sizeof(servaddr))) < 0){
		NvDebugLog("m_listenfd bind error.");
		return result;
	}

	if( (result = listen(m_listenfd, LISTENQ)) < 0){
		NvDebugLog("listen m_listenfd error.");
		return result;
	}

	if(pthread_create(&tid, NULL, handleAccept, this) != 0)
 	{
			NvDebugLog("pthread create failed!");
			return -1;
	}
...
#if 0
int CNvHWEncoder::Init_tcp_connect()
{
	struct sockaddr_in servaddr;
	int result = 0;
	if( (m_sockfd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0){
		NvDebugLog("socket create failed.");
		return m_sockfd;
	}
	bzero(&servaddr, sizeof(servaddr));
	servaddr.sin_family = AF_INET;
	servaddr.sin_port   = htons(8000);
	if ( (result = inet_pton(AF_INET, "192.168.2.50", &servaddr.sin_addr)) <= 0){
		NvDebugLog("inet_pton failed.");
		return -1;
	}
#if 1
	if( (result = connect(m_sockfd, (struct sockaddr*) &servaddr, sizeof(servaddr))) < 0){
		NvDebugLog("connect failed");
		return -1;
	}
#endif

#if 0
	while( (result = connect(m_sockfd, (struct sockaddr*) &servaddr, sizeof(servaddr))) < 0){
		NvDebugLog("connect failed, close socket.");
		close(m_sockfd);
		if( (m_sockfd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0){
			NvDebugLog("socket create failed.");
			return m_sockfd;
...
NVENCSTATUS CNvHWEncoder::ValidateEncodeGUID (GUID inputCodecGuid)
{
    ...
    if (nvStatus != NV_ENC_SUCCESS)
    {
        NvDebugLog("nvEncGetEncodeGUIDCount failed. nvstatus: %d", nvStatus);
        return nvStatus;
    }
    ...
    if (nvStatus != NV_ENC_SUCCESS)
    {
		...
		NvDebugLog("nvEncGetEncodeGUIDS failed. nvStatus: %d", nvStatus);
        return nvStatus;
    }
...
NVENCSTATUS CNvHWEncoder::CreateEncoder(EncodeConfig *pEncCfg)
{
    ...
    if ((m_uCurWidth > m_uMaxWidth) || (m_uCurHeight > m_uMaxHeight)) {
		NvDebugLog("m_uCurWidth > m_uMaxWidth or height");
        return NV_ENC_ERR_INVALID_PARAM;
    }
    ...
    {
    	NvDebugLog("width or height is 0");
        return NV_ENC_ERR_INVALID_PARAM;
    }
    ...
    if (nvStatus != NV_ENC_SUCCESS)
    {
        NvDebugLog("codec not supported, status: %d", nvStatus);
        return nvStatus;
    }
    ...
    if (nvStatus != NV_ENC_SUCCESS)
    {
        NvDebugLog("nvEncGetEncodePresetConfig returned failure, nvStatus: %d", nvStatus);
        return nvStatus;
    }
    ...
    if (pEncCfg->enableMEOnly == 1 || pEncCfg->enableMEOnly == 2)
    {
        ...
        if (nvStatus != NV_ENC_SUCCESS)
        {
            NvDebugLog("Encode Session Initialization failed, nvStatus: %d", nvStatus);
            return nvStatus;
        }
        else
        {
            if (meonlyMode == 1)
            {
                NvDebugLog("NV_ENC_CAPS_SUPPORT_MEONLY_MODE  supported");
            }
            else
            {
                NvDebugLog("NV_ENC_CAPS_SUPPORT_MEONLY_MODE not supported");
    ...
    if (pEncCfg->enableTemporalAQ == 1)
    {
        ...
        if (nvStatus != NV_ENC_SUCCESS)
        {
            NvDebugLog("Encode Session Initialization failed");
            return nvStatus;
        }
        else
        {
            ...
            {
                NvDebugLog("NV_ENC_CAPS_SUPPORT_TEMPORAL_AQ not supported");
                return NV_ENC_ERR_UNSUPPORTED_DEVICE;
            }
        }
    }
...
GUID CNvHWEncoder::GetPresetGUID(char* encoderPreset, int codec)
{
    ...
    {
        if (encoderPreset)
            NvDebugLog("Unsupported preset guid %s", encoderPreset);
...
NVENCSTATUS CNvHWEncoder::ProcessOutput(const EncodeBuffer *pEncodeBuffer)
{
    ...
    if (pEncodeBuffer->stOutputBfr.bWaitOnEvent == TRUE)
    {
        ...
        {
        	NvLog("bWaitOnEvent True and hOutputEvent is NULL.");
            return NV_ENC_ERR_INVALID_PARAM;
        }
#if defined(NV_WINDOWS)
    ...
    if (nvStatus == NV_ENC_SUCCESS)
    {
        ...
        //memcpy(m_out_buffer, lockBitstreamData.bitstreamBufferPtr, lockBitstreamData.bitstreamSizeInBytes);
#if 1
        frame_data_header_t header_data;
		memset(&header_data, 0, sizeof(header_data));
		header_data.length = lockBitstreamData.bitstreamSizeInBytes;
		header_data.width  = m_uCurWidth;
		header_data.height = m_uCurHeight;
		if(m_connfd > 0){
			if(writen(m_connfd, (void*)&header_data, sizeof(header_data)) < 0){
				NvDebugLog("writen header_data error.");
				close(m_connfd);
				m_connfd = 0;
			}
		}
		NvDebugLog("length: %d, width: %d, height: %d, m_connfd: %d", header_data.length, header_data.width, header_data.height, m_connfd);
		
		if(m_connfd > 0){
			if(writen(m_connfd, (void*)lockBitstreamData.bitstreamBufferPtr, lockBitstreamData.bitstreamSizeInBytes) < 0){
				NvDebugLog("writen body_data error.");
				close(m_connfd);
				m_connfd = 0;
			}
#if 0
			if(pro_cnt % 30 == 0 && wrt_cnt < 500){
				fwrite(lockBitstreamData.bitstreamBufferPtr, lockBitstreamData.bitstreamSizeInBytes, 1, h264);
				wrt_cnt++;
			}
#endif
		}
    ...
    {
        NvLog("lock bitstream function failed.");
    }
	pro_cnt++;
    return nvStatus;
}
...
NVENCSTATUS CNvHWEncoder::Initialize(void* device, NV_ENC_DEVICE_TYPE deviceType)
{
    ...
    if (nvEncodeAPICreateInstance == NULL){
		NvDebugLog("nvEncodeAPI.dll nvEncodeAPICreateInstance GetProcAddress failed");
		return NV_ENC_ERR_OUT_OF_MEMORY;
	}
    ...
    if (nvStatus != NV_ENC_SUCCESS){
		NvDebugLog("call nvEncodeAPICreateInstance failed, status: %d",nvStatus);
        return nvStatus;
	}

    nvStatus = NvEncOpenEncodeSessionEx(device, deviceType);
    if (nvStatus != NV_ENC_SUCCESS){
		NvDebugLog("open encode session failed, status: %d", nvStatus);
		return nvStatus;
	}
                                           ...
                                           int8_t *qpDeltaMapArray, uint32_t qpDeltaMapArraySize)
{
    ...
    {
    	NvDebugLog("nvEncEncodePicture failed. nvStatus: %d", nvStatus);
        return nvStatus;
    }
...
NVENCSTATUS CNvHWEncoder::ParseArguments(EncodeConfig *encodeConfig, int argc, char *argv[])
{
    for (int i = 1; i < argc-1; i++)
    {
		...
		else if(stricmp(argv[i], "-sip") == 0)
		{
			...
			encodeConfig->sip = argv[i];
		}
		else if(stricmp(argv[i], "-sport") == 0)
		{
			if(++i >= argc || sscanf(argv[i], "%d", &encodeConfig->servport) != 1)
			{
				PRINTERR("invalid parameter for %s\n", argv[i - 1]);
				return NV_ENC_ERR_INVALID_PARAM;
			}
        ...
        else if (stricmp(argv[i], "-inputFormat") == 0)
        {
            ...
            NV_ENC_BUFFER_FORMAT aFormatTable[] = { NV_ENC_BUFFER_FORMAT_NV12, NV_ENC_BUFFER_FORMAT_YUV444, NV_ENC_BUFFER_FORMAT_YUV420_10BIT, NV_ENC_BUFFER_FORMAT_YUV444_10BIT, NV_ENC_BUFFER_FORMAT_ARGB, NV_ENC_BUFFER_FORMAT_ABGR};
